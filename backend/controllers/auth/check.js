...
module.exports = async (req, res, next) => {
    ...
    const currentAccess = jwt.verify(access, process.env.JWT_SECRET);
    ...
    if(!currentAccess) {
        ...
        const user = await pool.query(`SELECT user_id, isActive FROM users WHERE user_id=$1`, [isTokenActive.rows[0].user_id]);
        ...
        if(user.rowCount === 0) {
            ...
            res.locals.auth = { token: accessToken }
            next();
        }
    ...
    console.log(currentAccess.user_id);

    const accessToken = jwt.sign(
        {
            type: 'access',
            user_id: currentAccess.user_id
        },
        process.env.JWT_SECRET,
        {
            expiresIn: '15m'
